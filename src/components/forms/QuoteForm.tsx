  ...
  return (
    <section className="surface-card" aria-labelledby="quote-form-title">
      ...
      <p className="mt-2 text-sm text-slate-300">
        Tell us what you are building. We respond with scope, timeline, and production guidance.
      </p>

      {submitted ? (
        <p className="mt-4 rounded-lg border border-support/50 bg-support/20 px-4 py-3 text-sm text-white">
          Quote request draft captured. Connect this form to your backend endpoint to activate live submission.
        </p>
      ) : null}

      <form className="mt-6 grid gap-5 sm:grid-cols-2" noValidate onSubmit={onSubmit}>
        ...
        <label className="block text-sm font-semibold text-slate-200" htmlFor="service">
          Service Needed
          <select
            id="service"
            className="field"
          ...
          {errors.service ? (
            <span id="service-error" className="mt-2 block text-xs text-red-300">
              {errors.service}
            </span>
          ) : null}
        </label>

        <label className="block text-sm font-semibold text-slate-200 sm:col-span-2" htmlFor="details">
          Project Details
          <textarea
            id="details"
            rows={5}
          ...
          />
          {errors.details ? (
            <span id="details-error" className="mt-2 block text-xs text-red-300">
              {errors.details}
            </span>
          ) : null}
        </label>

        <label className="block text-sm font-semibold text-slate-200" htmlFor="timeline">
          Timeline
          <input
            id="timeline"
            className="field"
          ...
          />
          {errors.timeline ? (
            <span id="timeline-error" className="mt-2 block text-xs text-red-300">
              {errors.timeline}
            </span>
          ) : null}
        </label>

        <label className="block text-sm font-semibold text-slate-200" htmlFor="budget">
        ...
        <label className="block text-sm font-semibold text-slate-200 sm:col-span-2" htmlFor="file-upload">
          File Upload (optional)
          <input
            id="file-upload"
            type="file"
            className="field file:mr-3 file:rounded-md file:border-0 file:bg-accent file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white"
            onChange={(event) => {
          ...
          />
          {values.fileName ? (
            <span className="mt-2 block text-xs text-slate-400">Attached: {values.fileName}</span>
          ) : null}
        </label>
