        ...
        <article className="surface-card">
          ...
          <ul className="mt-4 space-y-2 text-sm text-slate-300">
            {service.useCases.map((useCase) => (
              <li
                key={useCase}
                className="rounded-md border border-accentSoft/30 px-3 py-2"
              >
                {useCase}
              </li>
            ))}
        ...
        <article className="surface-card">
          ...
          <ul className="grid gap-2 sm:grid-cols-2">
            {service.materials.map((material) => (
              <li
                key={material}
                className="rounded-md border border-accentSoft/30 px-3 py-2 text-sm text-slate-200"
              >
                {material}
              </li>
            ))}
        ...
        <div className="rounded-2xl border border-accentSoft/35 bg-accent/20 p-6 sm:p-8 lg:flex lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-200">
              Send files, target quantity, and timing. We will return a
